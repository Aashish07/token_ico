...
contract('DappToken', function(accounts) {
	...
	it('transfers token ownership', () => {
		return DappToken.deployed().then(function(instance){
			...
			assert.equal(receipt.logs[0].args._from, accounts[0],'logs the accounts the tokens are transferred from');
			...
			assert.equal(balance.toNumber(),750000, 'deducts the amount from sending amount')
		});
	});
	...
	it('handles delicated token transfer',function() {
		return DappToken.deployed().then(function(instance){
			 tokenInstance = instance;
			...
			assert.equal(receipt.logs.length,1,'triggers one event');
			assert.equal(receipt.logs[0].event,'Transfer','should be the "Transfer" event');
			assert.equal(receipt.logs[0].args._from, fromAccount,'logs the accounts the tokens are transferred from');
